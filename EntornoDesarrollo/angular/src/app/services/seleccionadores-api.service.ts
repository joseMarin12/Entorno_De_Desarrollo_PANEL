...
export class SeleccionadoresApiService extends BaseCrud<Seleccionador> {
  ...
  // 🟢 URL dinámica apuntando al Proxy de Laravel en Cloud Run (entorno deployment)
  public override readonly API_URL = `${environment.apiUrl}/api/seleccionadores`;

  /**
   * Obtiene y sanitiza el token JWT del localStorage
   * Soporta tokens guardados como strings planos o como objetos/arreglos JSON.
   */
  ...
  findAll(page = 1, limit = 10, searchText = '', status = '', tipo = ''): Observable<SeleccionadorPage> {
    return this.http.post<{ data: any[] }>(
      ...
      map(res => {
        ...
        // Si la consulta no devuelve registros reales (objeto fantasma de SQL)
  ...
  create(data: Omit<Seleccionador, 'id'>): Observable<Seleccionador> {
    ...
    return this.http.post<Seleccionador>(
      this.API_URL,
      { action: 'createSeleccionador', seleccionadorData },
      this.getOptionsWithAuth()
    );
  }
  ...
  update(id: number, data: Partial<Seleccionador>): Observable<Seleccionador> {
    ...
    return this.http.post<Seleccionador>(
      this.API_URL,
      { action: 'updateSeleccionador', seleccionadorId: id, seleccionadorData },
      this.getOptionsWithAuth()
    );
  }
  ...
  toggleStatus(id: number): Observable<Seleccionador> {
    return this.http.post<Seleccionador>(
      this.API_URL,
      { action: 'toggleSeleccionadorStatus', seleccionadorId: id },
      this.getOptionsWithAuth()
    );
  }

  getEmpresas(): Observable<{ id: number; nombre: string }[]> {
    return this.http.post<{ data: { id: number; nombre: string }[] }>(
      this.API_URL,
      { action: 'getEmpresas' },
      this.getOptionsWithAuth()
    ).pipe(
